...
const GameType kGameType{/*short_name=*/"setback",
                         /*long_name=*/"Setback",
...
std::shared_ptr<const Game> Factory(const GameParameters& params) {
  return std::shared_ptr<const Game>(new SetbackGame(params));
}

REGISTER_SPIEL_GAME(kGameType, Factory);
}  // namespace

class SetbackObserver : public Observer {
 public:
     SetbackObserver(IIGObservationType iig_obs_type)
      : Observer(/*has_string=*/true, /*has_tensor=*/true),
        iig_obs_type_(iig_obs_type) {}

  void WriteTensor(const State& observed_state, int player,
                   Allocator* allocator) const override {
    const SetbackState& state =
        open_spiel::down_cast<const SetbackState&>(observed_state);
...
SetbackState::SetbackState(std::shared_ptr<const Game> game)
    : State(game),
...
int SetbackState::CurrentPlayer() const {
  if (IsTerminal()) {
...
void SetbackState::DoApplyAction(Action move) {
...
std::vector<Action> SetbackState::LegalActions() const {
...
std::string SetbackState::ActionToString(Player player, Action move) const {
...
std::string SetbackState::ToString() const {
...
bool SetbackState::IsTerminal() const { return winner_ != kInvalidPlayer; }

std::vector<double> SetbackState::Returns() const {
  if (!IsTerminal()) {
...
std::string SetbackState::InformationStateString(Player player) const {
  const SetbackGame& game = open_spiel::down_cast<const SetbackGame&>(*game_);
  return game.info_state_observer_->StringFrom(*this, player);
}

std::string SetbackState::ObservationString(Player player) const {
  const SetbackGame& game = open_spiel::down_cast<const SetbackGame&>(*game_);
  return game.default_observer_->StringFrom(*this, player);
}

void SetbackState::InformationStateTensor(Player player,
...
void SetbackState::ObservationTensor(Player player,
                                  absl::Span<float> values) const {
  ContiguousAllocator allocator(values);
  const SetbackGame& game = open_spiel::down_cast<const SetbackGame&>(*game_);
...
std::unique_ptr<State> SetbackState::Clone() const {
  return std::unique_ptr<State>(new SetbackState(*this));
}

void SetbackState::UndoAction(Player player, Action move) {
  if (history_.size() <= num_players_) {
    // Undoing a deal move.
...
std::vector<std::pair<Action, double>> SetbackState::ChanceOutcomes() const {
...
bool SetbackState::DidBet(Player player) const {
...
std::unique_ptr<State> SetbackState::ResampleFromInfostate(
...
SetbackGame::SetbackGame(const GameParameters& params)
    : Game(kGameType, params), num_players_(ParameterValue<int>("players")) {
  SPIEL_CHECK_GE(num_players_, kGameType.min_num_players);
  SPIEL_CHECK_LE(num_players_, kGameType.max_num_players);
  default_observer_ = std::make_shared<SetbackObserver>(kDefaultObsType);
  info_state_observer_ = std::make_shared<SetbackObserver>(kInfoStateObsType);
  private_observer_ = std::make_shared<SetbackObserver>(
      IIGObservationType{.public_info = false,
                         .perfect_recall = false,
                         .private_info = PrivateInfoType::kSinglePlayer});
  public_observer_ = std::make_shared<SetbackObserver>(
      IIGObservationType{.public_info = true,
                         .perfect_recall = false,
                         .private_info = PrivateInfoType::kNone});
}

std::unique_ptr<State> SetbackGame::NewInitialState() const {
  return std::unique_ptr<State>(new SetbackState(shared_from_this()));
}

std::vector<int> SetbackGame::InformationStateTensorShape() const {
...
std::vector<int> SetbackGame::ObservationTensorShape() const {
  // One-hot for whose turn it is.
  // One-hot encoding for the single private card. (n+1 cards = n+1 bits)
...
double SetbackGame::MaxUtility() const {
...
double SetbackGame::MinUtility() const {
  // In poker, the utility is defined as the money a player has at the end
  // of the game minus then money the player had before starting the game.
...
std::shared_ptr<Observer> SetbackGame::MakeObserver(
    absl::optional<IIGObservationType> iig_obs_type,
    const GameParameters& params) const {
  if (!params.empty()) SpielFatalError("Observation params not supported");
  return std::make_shared<SetbackObserver>(iig_obs_type.value_or(kDefaultObsType));
}
